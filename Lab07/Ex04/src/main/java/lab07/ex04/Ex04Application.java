...
public class Ex04Application {
	...
	public CommandLineRunner dataLoader(StudentRepository studentRepository, StudentService studentService) {
		return args -> {
			...
			List<Student> greaterEqual22 = studentService.getStudentsWithAgeGreaterThanEqual(22);
			System.out.println("Students with age >= 22:");
			greaterEqual22.forEach(System.out::println);

			long count = studentService.countStudentsByIeltsScore(7.5);
			System.out.println("Number of students with IELTS score 7.5: " + count);

			List<Student> nameSearchResults = studentService.getStudentsByNameContaining("Doe");
